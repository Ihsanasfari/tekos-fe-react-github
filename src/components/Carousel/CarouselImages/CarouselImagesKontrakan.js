...
const CarouselImagesKontrakan = (props) => {
  const { onClose } = props;
  const { kontrakanById } = useSelector((state) => state.kontrakanById);
  const [currentIndex, setCurrentIndex] = useState(0);

  const handleNext = () => {
    setCurrentIndex(
      (prevIndex) => (prevIndex + 1) % kontrakanById?.data?.image.length
    );
  };
  ...
  const handlePrev = () => {
    setCurrentIndex(
      (prevIndex) =>
        (prevIndex - 1 + kontrakanById?.data?.image.length) %
        kontrakanById?.data?.image.length
    );
  };
      ...
      >
        <div className="flex relative p-4 bg-slate-300 rounded-lg">
          ...
          <img
            ...
            src={kontrakanById?.data?.image[currentIndex]}
            alt="carouselKontrakan"
          />
          ...
          <div className="absolute top-0 left-0 z-30 flex items-center justify-center h-full px-4  group focus:outline-none">
            <button
              ...
              onClick={handlePrev}
              id="prevImageButton"
              aria-label="Toggle Previuos Image"
            >
              <MdArrowBackIosNew />
...
export default CarouselImagesKontrakan;
